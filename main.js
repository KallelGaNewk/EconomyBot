...
client.once('ready', () => {
  Utils.logger.success(
    `Discord instance ready in user ${client.user.tag} (${client.user.id})`,
  );
});
...
client.on('message', async (message) => {
  ...
  if (command.guildOnly && message.channel.type === 'dm')
    return message.reply(
      '**<:error:755511540320436385> Use commands in guild channel!**',
    );
  ...
  if (timestamps.has(message.author.id)) {
    ...
    if (now < expirationTime) {
      ...
      return message.reply(
        `**<:error:755511540320436385> Please wait ${timeLeft.toFixed(
          1,
        )} more second(s) before reusing the \`${prefix}${
          command.name
        }\` command.**`,
      );
    }
